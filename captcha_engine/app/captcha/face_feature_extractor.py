...
import numpy as np
# NOTE: cv2 는 extract_from_landmarks 경로(랜드마크를 이미 받은 경로)에서는 절대
# 호출되지 않는다. 모듈레벨 import 가 requirements.txt 에 없는 opencv 를 강제하면
# 엔진 전체가 ImportError 로 죽는다. mediapipe(_ensure_face_mesh 내부 import)와
# 동일 lazy 패턴으로 _extract_one_raw 메서드 내부로 이동했다. 동작 무변경.
...
class FaceFeatureExtractor:
    ...
    def _extract_one_raw(self, frame_bgr: np.ndarray) -> dict[str, float] | None:
        ...
        # cv2 lazy import — extract_from_landmarks 경로는 여기까지 오지 않으므로
        # opencv 가 requirements 에 없어도 엔진은 정상 동작한다. extract_from_frames
        # 를 쓰는 호출자만 opencv 가 필요.
        import cv2
