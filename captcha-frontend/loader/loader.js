...
// ---------------------------------------------------------------------------
// ★ 추가: 로컬 스토리지 기반 실패 횟수 및 밴(Ban) 제어 함수
// ---------------------------------------------------------------------------
function getFailData() {
  try {
    var count = parseInt(localStorage.getItem('_agami_fail_count'), 10) || 0;
    var banUntil = parseInt(localStorage.getItem('_agami_ban_until'), 10) || 0;
    return { count: count, banUntil: banUntil };
  } catch(e) { return { count: 0, banUntil: 0 }; }
}
...
function recordFail() {
  try {
    ...
    localStorage.setItem('_agami_fail_count', newCount);
    if (newCount >= 5) {
      localStorage.setItem('_agami_ban_until', Date.now() + 30 * 60 * 1000); // 30분 차단
    }
    return newCount;
  } catch(e) { return 1; }
}
...
function clearFails() {
  try {
    localStorage.removeItem('_agami_fail_count');
    localStorage.removeItem('_agami_ban_until');
  } catch(e) {}
}
...
function checkBan() {
  ...
  if (data.banUntil && Date.now() < data.banUntil) {
    return Math.ceil((data.banUntil - Date.now()) / 60000); // 남은 시간(분)
  }
  return 0;
}
...
function makeTrigger(onClick, theme) {
  ...
  b.innerHTML =
    ...
    '</span>' +
    '<span style="flex:1;font:700 16px system-ui,-apple-system,sans-serif;color:' + red + ';">' + msg + '</span>';
    
  if (!isBanned) {
    v.onmouseenter = function () {
      v.style.transform = 'translateY(-1px)';
      if (!dark) { v.style.borderColor = '#fb7185'; v.style.boxShadow = '0 2px 12px rgba(225,29,72,.14)'; }
    };
    v.onmouseleave = function () {
      v.style.transform = '';
      if (!dark) { v.style.borderColor = '#fecdd3'; v.style.boxShadow = ''; }
    };
    v.onclick = function() {
      api.reset(w.id);
    };
...
function showFailed(w, isBanned, minLeft) {
  removeVerified(w); 
  removeFailed(w); 
  w.failedEl = makeFailed(w.theme, isBanned, minLeft, w);
  w.div.appendChild(w.failedEl);
}

function removeFailed(w) {
  if (w.failedEl) { removeEl(w.failedEl); w.failedEl = null; }
}

// 실패 처리 통합 로직
function handleFail(w, reason) {
  recordFail();
  var minLeft = checkBan();
  var isBanned = minLeft > 0;
  
  w.token = '';
  setHidden(w, '');
  if (w.errorCallback) {
    try { w.errorCallback({ success: false, reason: reason }); } catch (e) { warn('errorCallback 예외: ' + e); }
  }
  
  removeIframe(w); // 팝업 모달창 제거
  if (w.triggerBtn) w.triggerBtn.style.display = 'none'; // 기본 트리거 버튼 숨김
  showFailed(w, isBanned, minLeft); // ❌ 실패 UI 출력
  
  w.phase = 'failed';
  setStatus(w, isBanned ? '5회 연속 실패로 30분간 차단되었습니다.' : '확인에 실패했습니다. 창을 닫고 다시 시도해주세요.');
}
...
function renderInto(div, opts) {
  ...
  w.triggerBtn = makeTrigger(function () {
    ...
    if (minLeft > 0) {
      ...
      showFailed(w, true, minLeft);
      w.phase = 'failed';
      setStatus(w, '5회 연속 차단 상태입니다.');
      return;
    }
  ...
  switch (data.type) {
    case 'agami-result':
      if (data.success) {
        ...
        handleFail(w, '캡챠 검증 실패'); // 모달창을 닫고 붉은색 실패 UI 표시
      }
      break;
    case 'agami-ready': 
      clearSpinner(w);
...
// ---------------------------------------------------------------------------
// ★ 추가: 개발자 도구 (급격한 화면 비율 변경) 감지 로직
// ---------------------------------------------------------------------------
var _lastInnerW = window.innerWidth;
...
window.addEventListener('resize', function() {
  ...
  // 모바일 기기 회전(Portrait <-> Landscape) 시에는 예외 처리
  var isRotation = Math.abs(currW - _lastInnerH) < 50 && Math.abs(currH - _lastInnerW) < 50;
  
  var wDrop = _lastInnerW - currW;
  var hDrop = _lastInnerH - currH;
  
  // 가로 또는 세로 크기가 150px 이상 급격히 축소되면 개발자 도구 창 등을 연 것으로 간주
  if (!isRotation && (wDrop > 150 || hDrop > 150)) {
    for (var key in widgets) {
      if (widgets.hasOwnProperty(key)) {
        var w = widgets[key];
        // 캡차 모달이 열려있는 상태에서만 감지하여 강제 실패 처리
        if (w.phase === 'expanded') {
          handleFail(w, '비정상적인 화면 비율 변경 감지');
        }
      }
    }
  }
...
var api = {
  ...
  reset: function (widgetId) {
    try {
      ...
      removeIframe(w); 
      removeVerified(w);
      removeFailed(w); // 실패 UI도 제거
      if (w.triggerBtn) w.triggerBtn.style.display = ''; // 트리거 버튼 숨김 해제
      w.phase = 'idle';
